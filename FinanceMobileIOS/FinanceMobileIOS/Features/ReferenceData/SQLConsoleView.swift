    ...
    private func executeSQL() {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            for (i, query) in queries.enumerated() {
                ...
                let outcome = TransactionRepository.shared.executeSQL(query)
                switch outcome {
                    ...
                    ScrollView(.horizontal, showsIndicators: true) {
                        VStack(alignment: .leading, spacing: 0) {
                            ...
                            ForEach(Array(result.rows.enumerated()), id: \.offset) { idx, row in
                                HStack(spacing: 0) {
                                    ForEach(Array(result.columns.enumerated()), id: \.offset) { colIdx, col in
                                        Text(row[col] ?? "NULL")
                                            .font(.system(.caption, design: .monospaced))
                                            .frame(minWidth: 80, alignment: .leading)
                                            .padding(.horizontal, 8)
                                ...
                                .background(idx % 2 == 0 ? Color.clear : Color(.secondarySystemBackground).opacity(0.5))
                            }
                        }
                    }
                    .background(Color(.secondarySystemBackground))
