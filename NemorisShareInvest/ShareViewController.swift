...
//  NemorisShareInvest
//
//  AXE P (2026-07-22) — share extension « Portefeuille » : activée quand
//  l'utilisateur partage un relevé d'investissement (PDF, capture d'écran/image,
//  CSV, texte). Dépose le fichier dans la boîte de réception App Group et informe
//  l'utilisateur. AUCUN import silencieux : Nemoris ouvre l'import intelligent
//  pré-rempli à sa prochaine ouverture (une extension de partage ne PEUT PAS
//  lancer son app conteneur — restriction Apple, pas un choix).
//
...
//  Fichier jumeau de NemorisShareTransactions/ShareViewController.swift — seule
//  la `ShareConfig` diffère.
//
...
private enum ShareConfig {
    /// Clé App Group relue par l'app (= `PendingImportInbox.Kind.investment`).
    static let pendingPathKey = "nemoris.pendingInvestmentImportPath"
    /// Types acceptés, par ordre de préférence de chargement. Le parser
    /// d'`InvestmentPDFImportView` détecte lui-même le format (PDF / image OCR /
    /// CSV / texte).
    static let acceptedTypes: [UTType] = [.pdf, .image, .commaSeparatedText, .tabSeparatedText, .plainText, .data]
    static let fallbackExtension = "dat"
    static let processingText = "Réception du document…"
    static let doneTitle = "Document transmis à Nemoris"
    static let doneMessage = "Ouvrez Nemoris : l'import intelligent analysera le document (positions ou ordres) pour relecture et validation. Rien n'est enregistré sans vous."
}
